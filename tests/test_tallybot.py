...
from mrtallyman.db import delete_team_table, get_table_name, get_team_user
...
def post_as_slack(client, app, body, endpoint="event", timestamp=None, secret=None):
    ...
    signature = generate_signature(timestamp, secret, json.dumps(body, sort_keys=True))
    return client.post(
        "/slack/%s" % endpoint,
        json=body,
        headers={
...
def test_event_message_reset_as_user(requests_mock, client, app):
    users_info = requests_mock.post(
        ...
        json={"user": {"is_admin": False, "name": "USER"}},
    )
    post_message = requests_mock.post(
        "https://slack.com/api/chat.postMessage", json={"ok": True}
    )
...
def test_event_message_reset_as_admin(requests_mock, client, app):
    users_info = requests_mock.post(
        "https://slack.com/api/users.info", json={"user": {"is_admin": True}}
    )
    post_message = requests_mock.post(
        "https://slack.com/api/chat.postMessage", json={"ok": True}
    )
...
def test_event_message_leaderboard(create_team_user, requests_mock, client, app):
    ...
    users_info = requests_mock.post(
        "https://slack.com/api/users.info", json=json_callback
    )
    post_message = requests_mock.post(
        "https://slack.com/api/chat.postMessage", json={"ok": True}
    )
...
def test_event_reaction_added_banana_to_bot(
    requests_mock, create_team_user, client, app
):
    create_team_user("TEAM", "USER")
...
def test_event_reaction_added_troll_to_bot(
    requests_mock, create_team_user, client, app
):
    create_team_user("TEAM", "USER")
