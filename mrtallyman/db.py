    ...
    with db_cursor() as cursor:
        ...
        for team in teams:
            with db_cursor() as users_cursor:
                sql = "SELECT COUNT(*) AS `user_count` FROM `%s`" % get_table_name(
                    team["id"]
                )
...
def update_team_user(team_id, user_id, attribute, value, giver=None):
    ...
    if user:
        ...
        golden_received = (
            value > 0
            and team["golden_threshold"] is not None
            and user[attribute] >= team["golden_threshold"]
        )

        if golden_received:
            args["golden_received"] = user["golden_received"] + floor(
                user[attribute] / team["golden_threshold"]
            )
        ...
        if giver and value > 0:
            ...
            giver = "<@%s>" % giver
            post_message(
                team_id, "You received a :%s: from %s!" % (reward_emoji, giver), user_id
            )

        if golden_received:
            post_message(
                team_id,
                "You have earned %s :%s:! Your :%s: have been reset to %d."
                % (
                    golden_received,
                    team["golden_emoji"],
                    reward_emoji,
...
def create_team_user(team_id, user_id, **attrs):
    ...
    sql = (
        "INSERT INTO `%s`" % get_table_name(team_id)
        + " (`team_id`, `user_id`, `rewards_given`, `rewards_given_today`, `golden_received`, `rewards_received`, `trolls_given`, `trolls_given_today`, `trolls_received`) values (%(team_id)s, %(user_id)s, %(rewards_given)s, %(rewards_given_today)s, %(golden_received)s, %(rewards_received)s, %(trolls_given)s, %(trolls_given_today)s, %(trolls_received)s)"
    )
...
def update_team_config(team_id, **attrs):
    ...
    if team:
        sql = (
            ...
            + ", ".join([f"`{key}` = %({key})s" for key in attrs.keys()])
            + " WHERE `id` = %(id)s"
        )
        args = attrs
        args["id"] = team_id
